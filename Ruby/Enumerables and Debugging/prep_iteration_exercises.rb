...
class Array
  def bubble_sort!(&prc)
    ...
    until swapping
      ...
      length.times do |i|
        break if i + 1 == length
        next unless prc.call(self[i], self[i + 1]) == 1
...
def subwords(word, dictionary)
  result = []
  dictionary.each { |ele| result << ele if word.include?(ele) && !result.include?(ele) }
  result
end
